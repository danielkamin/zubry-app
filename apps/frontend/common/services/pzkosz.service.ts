  ...
  static async getInstance() {
    if (!PzkoszApiService.pzkoszApiServiceInstance) {
      ...
      try {
        ...
        apiRequestParams.append('seasonid', result.data.attributes.settings.seasonId.toString());
        apiRequestParams.append('groupid', result.data.attributes.settings.stages[0].id.toString());
        apiRequestParams.append('teamid', result.data.attributes.settings.teamId.toString());
      } catch (err) {
        console.error(err);
        apiRequestParams.append('key', process.env.PZKOSZ_API_KEY);
