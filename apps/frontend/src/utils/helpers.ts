...
import { DEFAULT_YOUTUBE_VIDEO_ID } from '@/utils';
import { TStrapiOrder } from '@/types';
...
export const extractIdFromYoutubeUrl = (url: string): string => {
  ...
  try {
    if (url.includes('youtu.be')) {
      ...
      return DEFAULT_YOUTUBE_VIDEO_ID; //default video id from zubry yt channel
    }
  } catch (err) {
    console.error(err);
    return DEFAULT_YOUTUBE_VIDEO_ID; //default video id from zubry yt channel
...
export const transformWhiteSpaceTextToValidHyphenPath = (title: string) =>
